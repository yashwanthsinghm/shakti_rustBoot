...
impl FlashWriterEraser {
    pub fn new() -> Self {
        let mut spi_inner = unsafe { SPIInner::new(SPI_OFFSET) };
        spi_inner.init();
...
impl FlashInterface for FlashWriterEraser {
    ...
    // fn hal_flash_write1(&mut self, address: usize, data: *const u8, len: usize) {
    //     let address = address as u32;
    //     let len = len as u32;
    //     let mut idx = 0u32;
    //     let mut src = data as u32;
    //     let mut dst = address as u32;
    //     //Unlock the FLASH
    //     while idx < len {
    ...
    //             let b = self.spi.flash_read(dst as u32);

    //             self.spi.flash_write_enable();
    //             // spi.flash_erase(0x00b0_0000);
    //             self.spi.flash_status_register_read();

    //             let z = self.spi.flash_write(dst as u32, src as u32);
    //             src = ((src as u32) + 4) ; // increment pointer by 4
    //             dst = ((dst as u32) + 4) ; // increment pointer by 4
    //             idx += 4;
    //         } else {
    ...
    //             dst = ((dst as u32) - offset) ; // subtract offset from dst addr
    //             unsafe {
    //                 val = self.spi.flash_read(dst as u32); // assign current val at dst to val
    ...
    //             let z = self.spi.flash_write(dst, val);

    //             src = ((src as u32) + 1); // increment pointer by 1
    //             dst = ((dst as u32) + 1);  // increment pointer by 1
    //             idx += 1;
    //         }
    //     }
    //     //Lock the FLASH
    ...
    // fn hal_flash_erase1(&mut self, addr: usize, len: usize) {
